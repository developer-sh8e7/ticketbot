      ...
      <section className="opus-card min-h-[590px] overflow-hidden p-0">
        {submittedId && !ownerMode ? (
          ...
          <form onSubmit={createRequest} className="request-form-card mx-auto grid max-w-3xl gap-8 p-5 sm:p-9">
            ...
            <label className="grid gap-2"><span className="font-arabic text-sm font-bold text-opus-text">هل عندك موعد محدد؟ <span className="font-normal text-opus-muted">(اختياري)</span></span><input maxLength={100} value={deadline} onChange={(e) => setDeadline(e.target.value)} placeholder="مثال: خلال شهر" className="input font-arabic" /></label>

            <button disabled={creating} className="project-submit-btn group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-opus-accent px-6 py-4 font-arabic text-base font-extrabold text-black transition hover:-translate-y-0.5 disabled:opacity-50">{creating ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} className="transition group-hover:rotate-12" />} أرسل فكرة مشروعك</button>
          </form>
        ) : selected ? (
