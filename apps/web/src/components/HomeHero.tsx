    ...
    <section className="relative -mt-28 mx-[calc(50%-50vw)] flex min-h-[100svh] w-screen items-center overflow-hidden px-4 pb-7 pt-24 sm:pb-10 sm:pt-28 md:px-8 lg:px-12 lg:pt-32">
      <div className="mx-auto grid w-full max-w-7xl items-center gap-3 sm:gap-5 lg:grid-cols-[1.04fr_0.96fr] lg:gap-8">
        ...
        >
          ...
          <motion.h1
            variants={item}
            className="mt-4 text-balance font-arabic text-[clamp(2.15rem,10.5vw,2.75rem)] font-extrabold leading-[1.24] tracking-tight text-[var(--color-text)] sm:mt-5 md:text-7xl md:leading-tight"
          >
          ...
          >
            <Link
              ...
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--color-accent)] px-6 py-3 font-arabic text-sm font-extrabold text-[var(--color-text)] shadow-[0_12px_28px_rgba(232,108,0,0.18)] transition hover:-translate-y-0.5 hover:bg-[var(--color-accent-2)] hover:text-white focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg)]"
            >
              اطلب موقعك أو تطبيقك <ArrowLeft size={16} />
            </Link>
            <a
              href="#packages"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]/70 px-6 py-3 font-arabic text-sm font-bold text-[var(--color-text)] transition hover:-translate-y-0.5 hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg)]"
            >
