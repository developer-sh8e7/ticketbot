...
export const PROJECT_REQUEST_FIELDS = 'id,requester_hash,requester_discord_id_enc,requester_name_enc,phone_enc,status,owner_unread,customer_unread,created_at,last_message_at';

export function canAccessProjectRequest(
  row: Pick<ProjectRequestRow, 'requester_hash'>,
  discordUserId?: string | null,
  guestToken?: string | null,
) {
  if (discordUserId && (isOwnerId(discordUserId) || row.requester_hash === hashField(discordUserId))) return true;
  return Boolean(guestToken && verifyFieldHash(guestToken, row.requester_hash));
}
...
export function publicProjectRequest(row: ProjectRequestRow) {
  return {
    id: row.id,
    requesterDiscordId: decryptField(row.requester_discord_id_enc) === '__guest__' ? null : decryptField(row.requester_discord_id_enc),
