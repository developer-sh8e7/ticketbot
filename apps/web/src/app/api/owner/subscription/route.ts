...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rateLimit(req, 'owner:subscription', 15, 60_000).allowed) return fail('rate_limited', 'محاولات كثيرة. انتظر دقيقة.', 429);
    ...
    if (!Number.isFinite(days) || days < 1 || days > 3650) return fail('bad_request', 'مدة الاشتراك يجب أن تكون بين 1 و 3650 يوماً.', 400);
    ...
    const { data: instance, error } = await supabase.rpc('provision_instance', {
      ...
      p_external_ref: `subscription:${guildId}:${productType}:${Date.now()}`,
    });
    if (error) {
    ...
    if (instanceId) {
      await supabase.from('bot_instances').update({ plan_type: 'paid', updated_at: new Date().toISOString() }).eq('id', instanceId);
    }

    await logWebsiteEvent({
      eventType: 'owner_grant_subscription',
      message: 'Owner added a paid subscription',
