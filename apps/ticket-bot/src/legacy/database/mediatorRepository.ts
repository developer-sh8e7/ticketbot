...
export interface MediatorHistoryRecord {
  ...
  user_id: string;
  username: string;
  action: 'assign' | 'promote' | 'remove' | 'update' | 'warn' | 'unwarn' | 'auto_remove';
  actor_id: string;
  actor_tag: string;
  reason: string | null;
...
export interface MediatorWarningRecord {
  id: string;
  guild_id: string;
  user_id: string;
  username: string;
...
const inMemoryWarnings: MediatorWarningRecord[] = [];
...
export class MediatorRepository {
  ...
  public async logHistory(input: {
    user_id: string;
    username: string;
    action: MediatorHistoryRecord['action'];
    actor_id: string;
    actor_tag: string;
    reason?: string;
    details?: Record<string, any>;
  ...
  private warningMemoryList(guildId: string, userId?: string, activeOnly = false): MediatorWarningRecord[] {
    return inMemoryWarnings
      .filter((warning) => warning.guild_id === guildId)
      .filter((warning) => !userId || warning.user_id === userId)
      .filter((warning) => !activeOnly || warning.active)
      .slice()
      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  }

  public async createWarning(input: {
    guild_id: string;
    user_id: string;
    username: string;
    reason: string;
    warned_by: string;
    warned_by_tag: string;
  }): Promise<MediatorWarningRecord> {
    const record: MediatorWarningRecord = {
      id: crypto.randomUUID(),
      ...
      warned_by_tag: input.warned_by_tag,
      active: true,
      removed_by: null,
      removed_by_tag: null,
      removed_at: null,
    ...
    try {
      const { data, error } = await this.supabase
        .from('mediator_warnings')
        .insert(record)
        .select('*')
        .single();

      if (error) {
        this.handleDbError(error, 'createWarning');
        inMemoryWarnings.push(record);
        return record;
      }

      return data as MediatorWarningRecord;
    } catch (err) {
      logger.error('Mediator warning create failed.', err);
      inMemoryWarnings.push(record);
      return record;
    }
  }

  public async listWarnings(guildId: string, userId?: string, activeOnly = false): Promise<MediatorWarningRecord[]> {
    try {
      ...
      if (error) {
        this.handleDbError(error, 'listWarnings');
        return this.warningMemoryList(guildId, userId, activeOnly);
      }

      return (data || []) as MediatorWarningRecord[];
    } catch (err) {
      logger.error('Mediator warning list failed.', err);
      return this.warningMemoryList(guildId, userId, activeOnly);
    }
  }

  public async countActiveWarnings(guildId: string, userId: string): Promise<number> {
    try {
      ...
      if (error) {
        this.handleDbError(error, `countActiveWarnings (${userId})`);
        return this.warningMemoryList(guildId, userId, true).length;
      }

      return count ?? 0;
    } catch (err) {
      logger.error('Mediator warning count failed.', err);
      return this.warningMemoryList(guildId, userId, true).length;
    }
  }

  public async dismissLatestWarning(input: {
    guild_id: string;
    user_id: string;
    removed_by: string;
    removed_by_tag: string;
    remove_reason?: string;
  }): Promise<MediatorWarningRecord | null> {
    const latest = (await this.listWarnings(input.guild_id, input.user_id, true))[0];
    if (!latest) return null;

    const updates = {
      active: false,
      removed_by: input.removed_by,
      removed_by_tag: input.removed_by_tag,
      removed_at: new Date().toISOString(),
      remove_reason: input.remove_reason || null,
    };
    ...
    try {
      ...
      if (error) {
        this.handleDbError(error, `dismissLatestWarning (${input.user_id})`);
        const index = inMemoryWarnings.findIndex((warning) => warning.id === latest.id);
        const updated: MediatorWarningRecord = { ...latest, ...updates };
        if (index >= 0) inMemoryWarnings[index] = updated;
        return updated;
      }

      return data as MediatorWarningRecord;
    } catch (err) {
      logger.error('Mediator warning dismiss failed.', err);
      const index = inMemoryWarnings.findIndex((warning) => warning.id === latest.id);
      const updated: MediatorWarningRecord = { ...latest, ...updates };
      if (index >= 0) inMemoryWarnings[index] = updated;
      return updated;
  ...
  public async dismissActiveWarningsForUser(input: {
    guild_id: string;
    user_id: string;
    removed_by: string;
    removed_by_tag: string;
    ...
    try {
      ...
      if (error) {
        this.handleDbError(error, `dismissActiveWarningsForUser (${input.user_id})`);
        for (const warning of inMemoryWarnings) {
          if (warning.guild_id === input.guild_id && warning.user_id === input.user_id && warning.active) {
            Object.assign(warning, updates);
          }
        }
      }
    } catch (err) {
      logger.error('Mediator warning dismiss all failed.', err);
      for (const warning of inMemoryWarnings) {
        if (warning.guild_id === input.guild_id && warning.user_id === input.user_id && warning.active) {
          Object.assign(warning, updates);
        }
