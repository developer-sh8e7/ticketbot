...
function showView(name, payload) {
  els.loadingView.classList.toggle("active", name === "loading");
  ...
  els.progressPhone.className = "progress-step " + (name === "phone" ? "active" : name === "done" ? "done" : name === "loading" ? "active" : "");
  els.progressLine.className = "progress-line " + (name === "phone" || name === "done" || name === "loading" ? "done" : "");
  if (name === "phone") {
...
async function sendOtp(button) {
  ...
  if (!validatePhoneField()) {
    setMessage(els.sendMessage, "اختر الدولة واكتب رقم الجوال الصحيح بدون مفتاح الدولة.", "error");
    return;
  }
  try {
    ...
    setMessage(els.sendMessage, "تم إرسال رمز التحقق برسالة نصية إلى " + data.phone + ".", "success");
...
if (els.discordButton) {
  els.discordButton.addEventListener("click", () => {
    showView("loading", {});
    setMessage(els.oauthMessage, "جاري تحويلك إلى Discord...", "");
  });
}
