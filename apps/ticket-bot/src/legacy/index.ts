...
import { OpusRepository } from './database/opusRepository.js';
...
import { OpusManagerService } from './services/opusManagerService.js';
import { BotInstanceManager } from './services/botInstanceManager.js';
import { LinkUserService } from './services/linkUserService.js';
import { TempRoomService } from './services/tempRoomService.js';
import { Voice247Service } from './services/voice247Service.js';
...
import {
  checkVouchesPermission,
  incrementVouchCount,
  VOUCHES_TARGET_CHANNEL_ID,
} from './services/vouchesService.js';
import { buildVouchImage, VOUCHES_REVIEW_MAX_LENGTH } from './services/vouchesImageService.js';
...
const opusRepository = new OpusRepository(supabase);
...
const opusManagerService = new OpusManagerService(opusRepository, env.TRIAL_MANAGER_ID, env.OPUS_NOTIFY_CHANNEL_ID);
const botInstanceManager = new BotInstanceManager(supabase, env);
const linkUserService = new LinkUserService(
  supabase,
  env.LINK_WEBHOOK_URL,
  configStore.current,
  env.FIELD_ENCRYPTION_KEY || env.JWT_SECRET,
  env.JWT_SECRET,
);
...
function isOpusRuntime(): boolean {
  return env.RUN_MODE === 'manager';
}
...
async function handleCommand(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const config = guildId ? configStore.get(guildId) : configStore.current;
  ...
  if (interaction.commandName === config.commands.names.configReload) {
    ...
    try {
      await infrastructureService.ensureInfrastructure(interaction.client, interaction.guild ?? undefined);
    } catch (error) {
  ...
  if (interaction.commandName === config.commands.names.emojiRefresh) {
    ...
    try {
      ...
      configStore.patchEmojis(guild.id, emojis.categoryEmojis, emojis.buttonEmojis);
  ...
  if (isOpusRuntime() && interaction.commandName === 'trial') {
    await opusManagerService.handleTrialCommand(interaction);
    return;
  }

  if (isOpusRuntime() && interaction.commandName === 'check') {
    await opusManagerService.handleCheckCommand(interaction);
    return;
  }
  ...
  if (interaction.commandName === '24-7') {
    await voice247Service.handleStart(interaction);
    return;
  }

  if (interaction.commandName === 'stop') {
    await voice247Service.handleStop(interaction);
    return;
  }
  ...
  if (interaction.commandName === 'mm') {
    await roleManagementService.sendPermissionPanel(interaction);
    return;
  }
  ...
  if (interaction.commandName === 'vouches') {
    if (!interaction.inCachedGuild() || !interaction.member) {
      await safeReply(interaction, [buildErrorEmbed(config, 'هذا الأمر يعمل فقط داخل السيرفر.')]);
      return;
    }

    const roleIds = [...interaction.member.roles.cache.keys()];
    const permission = checkVouchesPermission(roleIds, interaction.guildId, interaction.user.id);

    if (!permission.allowed) {
      const errEmbed = new EmbedBuilder()
        .setColor(Number(config.bot.errorColor?.replace('#', '0x') ?? 0xef4444))
    ...
    if (review.length > VOUCHES_REVIEW_MAX_LENGTH) {
      const errEmbed = new EmbedBuilder()
        .setColor(Number(config.bot.errorColor?.replace('#', '0x') ?? 0xef4444))
        .setDescription(`التقييم طويل جدًا. الحد الأقصى ${VOUCHES_REVIEW_MAX_LENGTH} حرف.`);
      await safeReply(interaction, [errEmbed]);
      return;
    }

    const rating = interaction.options.getInteger('rating') ?? 5;
    const customerName = interaction.member.displayName || interaction.user.displayName || interaction.user.username;
    ...
    try {
      const channel = await interaction.client.channels.fetch(VOUCHES_TARGET_CHANNEL_ID).catch(() => null);
      if (!channel?.isTextBased() || channel.isDMBased()) {
        const failEmbed = new EmbedBuilder()
          .setColor(Number(config.bot.errorColor?.replace('#', '0x') ?? 0xef4444))
          .setDescription('تعذر العثور على قناة التقييمات.');
        await safeReply(interaction, [failEmbed]);
        return;
      }

      const image = await buildVouchImage({ review, rating, customerName, avatarUrl });
      const attachment = new AttachmentBuilder(image, { name: 'vouch.png' });
      const embed = new EmbedBuilder()
        .setColor(0x1a1a1a)
        .setImage('attachment://vouch.png')
        .setTimestamp(new Date());

      const textChannel = channel as import('discord.js').TextChannel;
      await textChannel.send({ embeds: [embed], files: [attachment] });

      incrementVouchCount(interaction.guildId, interaction.user.id);

      const successEmbed = new EmbedBuilder()
        .setColor(Number(config.bot.successColor?.replace('#', '0x') ?? 0x22c55e))
        .setDescription('تم إرسال تقييمك بنجاح ✅');
      await safeReply(interaction, [successEmbed]);
    } catch (error) {
      logger.error('[vouches] Failed to send:', error instanceof Error ? error.message : error);
      const code = typeof error === 'object' && error !== null && 'code' in error ? Number((error as { code?: unknown }).code) : 0;
      const message = code === 50013
        ? 'البوت ما عنده صلاحية إرسال في قناة التقييمات.'
        : 'حدث خطأ أثناء إنشاء أو إرسال صورة التقييم. حاول مرة أخرى.';
      const failEmbed = new EmbedBuilder()
        .setColor(Number(config.bot.errorColor?.replace('#', '0x') ?? 0xef4444))
  ...
  if (interaction.commandName === 'info') {
    await linkUserService.handleInfoCommand(interaction);
    return;
  }
}
...
client.once(Events.ClientReady, async (readyClient) => {
  ...
  try {
    const allGuilds = await readyClient.guilds.fetch();
    for (const guild of allGuilds.values()) {
      const fullGuild = await readyClient.guilds.fetch(guild.id);
      await infrastructureService.ensureInfrastructure(readyClient, fullGuild);
    }
  ...
  try {
    ...
    for (const guild of guilds.values()) {
      const fullGuild = await readyClient.guilds.fetch(guild.id);
      const removed = await ticketService.cleanupStaleOpenTickets(fullGuild);
      if (removed > 0) {
        logger.warn(`Removed ${removed} stale open ticket record(s) during startup for guild ${guild.id}.`);
      }
    }
  } catch (error) {
  ...
  if (isOpusRuntime()) {
    try {
      await opusManagerService.checkExpiry(readyClient);
      setInterval(() => {
        opusManagerService.checkExpiry(readyClient).catch((error) => {
          logger.error('Opus expiry checker failed', error instanceof Error ? error.message : error);
        });
      }, 10 * 60 * 1000).unref();
      logger.info('Opus expiry checker started.');
    } catch (error) {
      logger.error('Failed to start Opus expiry checker', error instanceof Error ? error.message : error);
    }
  } else {
    logger.info('Opus expiry checker disabled outside RUN_MODE=manager.');
  }
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  const customId = interaction.isButton() || interaction.isModalSubmit() || interaction.isStringSelectMenu() || interaction.isUserSelectMenu()
  ...
  try {
    ...
    if ((interaction.isUserSelectMenu() || interaction.isRoleSelectMenu()) && roleManagementService.isPermissionPanelSelect(interaction.customId)) {
      await roleManagementService.handlePermissionSelect(interaction);
      trackLifecycleHealth();
      return;
    }

    if ((interaction.isStringSelectMenu() || interaction.isUserSelectMenu()) && tempRoomService.isTempSelect(interaction.customId)) {
      await tempRoomService.handleSelect(interaction);
      trackLifecycleHealth();
      return;
    }
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (tempRoomService.isTempModal(interaction.customId)) {
        await tempRoomService.handleModal(interaction);
        trackLifecycleHealth();
        return;
      }
    ...
    if (interaction.isButton()) {
      if (tempRoomService.isTempButton(interaction.customId)) {
        await tempRoomService.handleButton(interaction);
        trackLifecycleHealth();
        return;
      }

      if (interaction.customId === APPLY_MEDIATOR) {
        const guildConfig = interaction.guildId ? configStore.get(interaction.guildId) : configStore.current;
        if (!guildConfig.features.applicationsPanel) {
          await safeReply(interaction, [buildErrorEmbed(guildConfig, 'التقديم غير مفعل في هذا السيرفر.')]);
          trackLifecycleHealth();
          return;
        }
      ...
      if (interaction.customId === TRIAL_BUTTON_IDS.close) {
        await ticketService.handleTrialCloseButton(interaction);
        trackLifecycleHealth();
        return;
      }

      if (isOpusRuntime() && interaction.customId === TRIAL_BUTTON_IDS.accept) {
        await opusManagerService.handleTrialAccept(interaction);
        trackLifecycleHealth();
        return;
      }

      if (isOpusRuntime() && interaction.customId === TRIAL_BUTTON_IDS.reject) {
        await opusManagerService.handleTrialReject(interaction);
        trackLifecycleHealth();
        return;
      }

      if (isOpusRuntime() && (interaction.customId.startsWith(CHECK_BUTTON_PREFIXES.user) || interaction.customId.startsWith(CHECK_BUTTON_PREFIXES.guild))) {
        await opusManagerService.handleCheckButton(interaction);
        trackLifecycleHealth();
        return;
      }
      ...
      if (roleManagementService.isPermissionPanelButton(interaction.customId)) {
        await roleManagementService.handlePermissionButton(interaction);
        trackLifecycleHealth();
        return;
      }
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
  try {
    await tempRoomService.handleVoiceStateUpdate(oldState, newState).catch((error) => {
      logger.warn('Failed to handle temporary voice room update', error instanceof Error ? error.message : error);
    });
    await voice247Service.handleVoiceStateUpdate(oldState, newState).catch((error) => {
      logger.warn('Failed to handle 24/7 voice update', error instanceof Error ? error.message : error);
    });
...
client.on(Events.MessageCreate, async (message) => {
  try {
    ...
    if (isOpusRuntime()) {
      const trialKeywordHandled = await opusManagerService.handleKeywordMessage(message);
      if (trialKeywordHandled) return;
    }
...
app.post('/internal/bot-instances/sync', express.json({ limit: '64kb' }), async (req, res) => {
  if (env.MANAGER_SYNC_SECRET) {
    const provided = req.header('x-manager-sync-secret') ?? '';
    if (provided !== env.MANAGER_SYNC_SECRET) {
      res.status(401).json({ ok: false, error: 'unauthorized' });
      return;
    }
  }

  if (env.RUN_MODE !== 'manager') {
    res.status(409).json({ ok: false, error: 'not_manager_mode' });
    return;
  }

  try {
    await botInstanceManager.sync();
    res.status(200).json({ ok: true, running: botInstanceManager.runningCount, reason: req.body?.reason ?? null });
  } catch (error) {
    logger.error('Internal bot instance sync failed', error instanceof Error ? error.message : error);
...
async function startDiscordClient(): Promise<void> {
  ...
  if (!env.DISCORD_TOKEN) {
    if (env.RUN_MODE === 'manager') {
      logger.warn('RUN_MODE=manager started without OPUS_CONTROL_BOT_TOKEN/DISCORD_TOKEN; Discord client is disabled.');
      runtimeState = 'online';
      return;
    }
    throw new Error('DISCORD_TOKEN is required in legacy mode.');
  }
...
async function startAll(): Promise<void> {
  ...
  // In manager mode, start the BotInstanceManager to manage client bots
  if (env.RUN_MODE === 'manager') {
    try {
      await botInstanceManager.init();
      logger.info(`BotInstanceManager started — ${botInstanceManager.runningCount} client bot(s) running`);
    } catch (error) {
      logger.error('Failed to start BotInstanceManager', error instanceof Error ? error.message : error);
    }
  }
}
...
startAll().catch((error) => {
  runtimeState = 'offline';
  logger.error('Failed to start', error instanceof Error ? error.message : error);
});
