...
export class InfrastructureService {
  ...
  public async ensureInfrastructure(client: Client, guild?: Guild): Promise<void> {
    if (!guild) {
      logger.warn('No guild provided to ensureInfrastructure. Skipping.');
      return;
    }
    const guildId = guild.id;

    logger.info(`Ensuring infrastructure for guild: ${guild.name} (${guildId})`);
    ...
    this.configStore.patchGuild(guildId, {
    ...
    this.configStore.patchPanel(guildId, {
    ...
    try {
      ...
      this.configStore.patchEmojis(guildId, emojis.categoryEmojis, emojis.buttonEmojis);
