...
import { buttonStyleFromName, isGuildTextChannelType } from '../utils/discord.js';
...
export class TicketService {
  ...
  private getEnabledCategory(categoryKey: string): TicketCategoryConfig | undefined {
    return this.config.categories.find((category) => category.enabled && (category.key === categoryKey || category.customId === categoryKey));
  }
  ...
  private buildTrialActionRows(): ActionRowBuilder<ButtonBuilder>[] {
    const closeStyle = buttonStyleFromName(this.config.ticket.controls.close.style) as ButtonStyle;
    return [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
          .setCustomId(TRIAL_BUTTON_IDS.close)
          .setLabel('قفل التذكرة')
          .setStyle(closeStyle)
          .setEmoji({ id: '1517622263229452318' }),
        new ButtonBuilder()
          .setCustomId(TRIAL_BUTTON_IDS.accept)
          .setLabel('إعطاء التجربة')
          .setStyle(ButtonStyle.Secondary)
          .setEmoji({ id: '1517622243759362149' }),
        new ButtonBuilder()
          .setCustomId(TRIAL_BUTTON_IDS.reject)
          .setLabel('رفض التجربة')
          .setStyle(ButtonStyle.Secondary)
          .setEmoji({ id: '1505357865072984115' }),
      ),
    ];
  }
  ...
  public async restoreTicketPanel(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const refreshedPanel = existingPanel
      ? await existingPanel.edit({ content: '', embeds, files, attachments: [], components }).catch(() => null)
      : null;
  ...
  private async processTicketCreation(
    ...
    try {
      ...
      try {
        ...
        const isFreeTrialTicket = this.isFreeTrialCategory(category);
        const channelName = isFreeTrialTicket
          ? normalizeChannelName(`trial-${interaction.user.username}`, this.config.naming.maxChannelNameLength)
          : this.buildTicketChannelName(category, ticketNumber, answers);
        ...
        createdTicket = await this.ticketRepository.createTicket({
          ...
          metadata: {
            ...
            ...(isFreeTrialTicket ? { ticketType: 'free_trial' } : {}),
        ...
        let trialImageAttachment: AttachmentBuilder | null = null;
        if (isFreeTrialTicket) {
          const imageResponse = await fetch('https://i.imgur.com/fq4IOGJ.jpeg');
          const imageBuffer = Buffer.from(await imageResponse.arrayBuffer());
          trialImageAttachment = new AttachmentBuilder(imageBuffer, { name: 'opus-free-trial.jpeg' });
        }

        const sentMessage = isFreeTrialTicket
          ? await created.send({
              files: trialImageAttachment ? [trialImageAttachment] : [],
              components: this.buildTrialActionRows(),
            })
          : await created.send({
        ...
        if (isFreeTrialTicket) {
          try {
            await this.ticketRepository.client.from('service_events').insert({
              user_id: interaction.user.id,
              guild_id: interaction.guildId,
              event_type: 'trial_ticket_opened',
              event_message: 'Free trial ticket opened.',
              metadata: {
                channel_id: created.id,
            ...
            // service_events belongs to the optional Opus schema; ticket creation must not fail if it is missing.
          }
        }
  ...
  public async handleTrialCloseButton(interaction: ButtonInteraction): Promise<void> {
    if (!(await safeDeferReply(interaction, 'trial-close-ticket'))) {
      return;
    }
